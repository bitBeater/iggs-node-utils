...
(() => {
	...
	var Me = (t, e, n, r) => {
		if ((e && typeof e == 'object') || typeof e == 'function')
			for (let i of Ce(e)) !Re.call(t, i) && i !== n && ae(t, i, { get: () => e[i], enumerable: !(r = Pe(e, i)) || r.enumerable });
		return t;
	};
	var De = (t, e, n) => (
		(n = t != null ? Qe(Oe(t)) : {}), Me(e || !t || !t.__esModule ? ae(n, 'default', { value: t, enumerable: !0 }) : n, t)
	);
	var de = _e((ce, he) => {
		(function () {
			var t = function (e) {
...
`,
			...
			(t.Vector = function (e) {
				...
				(t.Vector.prototype.dot = function (e) {
					for (var n = 0, r = this.elements, i = e.elements, s = r.length, o = i.length, a = 0, u = 0, l = 0, h = 0; l < s && h < o; )
						(a = r[l]), (u = i[h]), a < u ? (l += 2) : a > u ? (h += 2) : a == u && ((n += r[l + 1] * i[h + 1]), (l += 2), (h += 2));
					return n;
				}),
						...
						if (
							...
							(g = L[1]),
								...
								x.test(g) &&
									...
									(O = _),
									(D = z),
									x.test(c) ? (c = c + 'e') : O.test(c) ? ((d = S), (c = c.replace(d, ''))) : D.test(c) && (c = c + 'e'));
						}
						if (((d = A), d.test(c))) {
							var L = d.exec(c);
							(g = L[1]), (c = g + 'i');
			...
			(t.TokenSet = function () {
						...
						if (s.editsRemaining != 0) {
							...
							if (s.str.length > 1) {
								var h = s.str.charAt(0),
									m = s.str.charAt(1),
									v;
								m in s.node.edges ? (v = s.node.edges[m]) : ((v = new t.TokenSet()), (s.node.edges[m] = v)),
				...
				(t.TokenSet.Builder.prototype.insert = function (e) {
					...
					this.minimize(r),
						this.uncheckedNodes.length == 0 ? (n = this.root) : (n = this.uncheckedNodes[this.uncheckedNodes.length - 1].child);
							...
							for (var w = 0; w < E.length; w++)
								for (var S = E[w], I = this.invertedIndex[S], _ = I._index, f = 0; f < l.fields.length; f++) {
									var p = l.fields[f],
										...
										$ = new t.Set(A);
									if (
										(l.presence == t.Query.presence.REQUIRED && ((m = m.union($)), o[p] === void 0 && (o[p] = t.Set.complete)),
					...
					for (var e = {}, n = Object.keys(this.fieldTermFrequencies), r = n.length, i = Object.create(null), s = 0; s < r; s++) {
						for (
							...
							i[f] === void 0 ? ((S = t.idf(this.invertedIndex[f], this.documentCount)), (i[f] = S)) : (S = i[f]),
								(I = (S * ((this._k1 + 1) * p)) / (this._k1 * (1 - this._b + this._b * (u / this.averageFieldLength[a])) + p)),
								(I *= b),
								(I *= y),
		...
		createComponents(e) {
			le.forEach(n => {
				e.querySelectorAll(n.selector).forEach(r => {
					r.dataset.hasInstance || (new n.constructor({ el: r }), (r.dataset.hasInstance = String(!0)));
				});
			});
		}
	};
	...
	var re = class extends J {
			...
			hideShowToolbar() {
				...
				(this.showToolbar =
					this.lastY >= this.scrollTop || this.scrollTop <= 0 || (!!this.searchInput && this.searchInput === document.activeElement)),
					n !== this.showToolbar &&
						(this.toolbar.classList.toggle('tsd-page-toolbar--hide'), this.navigation?.classList.toggle('col-menu--hide')),
	...
	function pe(t, e) {
		...
		let n = t.toLocaleLowerCase(),
			...
			o = n.indexOf(r);
		for (; o != -1; )
			i.push(ie(t.substring(s, o)), `<b>${ie(t.substring(o, o + r.length))}</b>`), (s = o + r.length), (o = n.indexOf(r, s));
...
`);
	...
	var te = class extends k {
		...
		expand(n = !0) {
			(this.el.open = !0), this.animate(this.collapsedHeight, this.expandedHeight, { opening: !0, duration: n ? 300 : 0 });
		}
		collapse(n = !0) {
			this.animate(this.expandedHeight, this.collapsedHeight, { opening: !1, duration: n ? 300 : 0 });
		}
		animate(n, r, { opening: i, duration: s = 300 }) {
			...
			(this.animation = this.el.animate({ height: [n, r] }, o)),
				this.icon
					.animate({ transform: [this.icon.style.transform || this.getIconRotation(!i), this.getIconRotation(i)] }, o)
