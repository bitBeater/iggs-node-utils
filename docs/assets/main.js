(() => {
	...
	var De = (t, e, r, n) => {
		if ((e && typeof e == 'object') || typeof e == 'function')
			for (let i of Oe(e)) !_e.call(t, i) && i !== r && ue(t, i, { get: () => e[i], enumerable: !(n = Pe(e, i)) || n.enumerable });
		return t;
	};
	var Fe = (t, e, r) => (
		(r = t != null ? Ce(Re(t)) : {}), De(e || !t || !t.__esModule ? ue(r, 'default', { value: t, enumerable: !0 }) : r, t)
	);
	var pe = Me((de, fe) => {
		(function () {
			var t = function (e) {
			...
			(t.utils = {}),
				...
				(t.FieldRef.prototype.toString = function () {
					return (
						this._stringValue == null && (this._stringValue = this.fieldName + t.FieldRef.joiner + this.docRef), this._stringValue
					);
				});
			(t.Set = function (e) {
...
`,
			...
			(t.Vector = function (e) {
				...
				(t.Vector.prototype.dot = function (e) {
					for (var r = 0, n = this.elements, i = e.elements, s = n.length, o = i.length, a = 0, l = 0, u = 0, h = 0; u < s && h < o; )
						(a = n[u]), (l = i[h]), a < l ? (u += 2) : a > l ? (h += 2) : a == l && ((r += n[u + 1] * i[h + 1]), (u += 2), (h += 2));
					return r;
				}),
			...
			(t.stemmer = (function () {
				var e = {
					...
					z = function (c) {
						...
						if (
							...
							(g = L[1]),
								...
								x.test(g) &&
									...
									(R = M),
									(F = B),
									x.test(c) ? (c = c + 'e') : R.test(c) ? ((d = w), (c = c.replace(d, ''))) : F.test(c) && (c = c + 'e'));
						}
						if (((d = V), d.test(c))) {
							var L = d.exec(c);
							(g = L[1]), (c = g + 'i');
						...
						if (((d = P), d.test(c))) {
							var L = d.exec(c);
							(g = L[1]), (d = p), (x = E), (R = G), (d.test(g) || (x.test(g) && !R.test(g))) && (c = g);
						}
						return (
							(d = U),
							(x = p),
			...
			(t.TokenSet = function () {
				...
				(t.TokenSet.prototype.intersect = function (e) {
					for (var r = new t.TokenSet(), n = void 0, i = [{ qNode: e, output: r, node: this }]; i.length; ) {
						n = i.pop();
						for (var s = Object.keys(n.qNode.edges), o = s.length, a = Object.keys(n.node.edges), l = a.length, u = 0; u < o; u++)
			...
			(t.Index = function (e) {
				...
				(t.Index.prototype.query = function (e) {
					...
					for (var l = 0; l < r.clauses.length; l++) {
						var u = r.clauses[l],
							h = null,
							f = t.Set.empty;
						u.usePipeline ? (h = this.pipeline.runString(u.term, { fields: u.fields })) : (h = [u.term]);
						for (var p = 0; p < h.length; p++) {
							...
							for (var T = 0; T < b.length; T++)
								for (var w = b[T], I = this.invertedIndex[w], M = I._index, m = 0; m < u.fields.length; m++) {
									var v = u.fields[m],
										B = I[v],
										V = Object.keys(B),
										q = w + '/' + v,
										$ = new t.Set(V);
									if (
										(u.presence == t.Query.presence.REQUIRED && ((f = f.union($)), o[v] === void 0 && (o[v] = t.Set.complete)),
				...
				(t.Index.prototype.toJSON = function () {
					...
					return { version: t.version, fields: this.fields, fieldVectors: r, invertedIndex: e, pipeline: this.pipeline.toJSON() };
				}),
			...
			(t.Builder = function () {
				...
				(t.Builder.prototype.add = function (e, r) {
					...
					for (var s = 0; s < i.length; s++) {
						...
						(this.fieldTermFrequencies[f] = p), (this.fieldLengths[f] = 0), (this.fieldLengths[f] += h.length);
						for (var E = 0; E < h.length; E++) {
							...
							for (var v = 0; v < this.metadataWhitelist.length; v++) {
								var T = this.metadataWhitelist[v],
									w = y.metadata[T];
								this.invertedIndex[y][o][n][T] == null && (this.invertedIndex[y][o][n][T] = []), this.invertedIndex[y][o][n][T].push(w);
							}
						}
					}
				}),
					...
					for (var e = {}, r = Object.keys(this.fieldTermFrequencies), n = r.length, i = Object.create(null), s = 0; s < n; s++) {
						for (
							...
							i[m] === void 0 ? ((w = t.idf(this.invertedIndex[m], this.documentCount)), (i[m] = w)) : (w = i[m]),
								(I = (w * ((this._k1 + 1) * v)) / (this._k1 * (1 - this._b + this._b * (l / this.averageFieldLength[a])) + v)),
								(I *= E),
								(I *= y),
				...
				(function (e, r) {
					typeof define == 'function' && define.amd ? define(r) : typeof de == 'object' ? (fe.exports = r()) : (e.lunr = r());
				})(this, function () {
					return t;
				});
		})();
	...
	function He(t, e, r, n) {
		...
		for (let l = 0; l < s.length; l++) {
			let u = s[l],
				...
				f = 1;
			h.name.toLowerCase().startsWith(i.toLowerCase()) && (f *= 1 + 1 / (Math.abs(h.name.length - i.length) * 10)),
		...
		for (let l = 0, u = Math.min(10, s.length); l < u; l++) {
			...
			(E.href = n.base + h.url), E.classList.add('tsd-kind-icon'), (E.innerHTML = f), p.append(E), e.appendChild(p);
		}
	}
	function me(t, e) {
		var n, i;
	...
	function ve(t, e) {
		...
		let r = t.toLocaleLowerCase(),
			...
			o = r.indexOf(n);
		for (; o != -1; )
			i.push(se(t.substring(s, o)), `<b>${se(t.substring(o, o + n.length))}</b>`), (s = o + n.length), (o = r.indexOf(n, s));
				...
				this.createGroups(),
					this.container &&
						...
						Array.from(this.el.children).forEach(n => {
							n.addEventListener('touchstart', i => this.onClick(i)), n.addEventListener('click', i => this.onClick(i));
						}),
						this.container.classList.add('active'),
						this.setIndex(0));
			}
			setIndex(r) {
				if ((r < 0 && (r = 0), r > this.groups.length - 1 && (r = this.groups.length - 1), this.index == r)) return;
				let n = this.groups[r];
				if (this.index > -1) {
	...
	Ee && 'ontouchstart' in document.documentElement && ((je = !0), (C = 'touchstart'), (Le = 'touchmove'), (_ = 'touchend'));
